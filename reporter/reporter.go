...
// SupplyResult holds the integrity check outcome for one event group.
type SupplyResult struct {
	...
	OveragePercent  float64 `json:"overagePercent"`
	Pass            bool    `json:"pass"`
}

// SupplySummary holds aggregate metrics for a supply integrity run.
type SupplySummary struct {
	TotalChecked     int     `json:"totalChecked"`
	PassCount        int     `json:"passCount"`
	FailCount        int     `json:"failCount"`
	MaxOverage       float64 `json:"maxOveragePct"`
	TolerancePct     float64 `json:"tolerancePct"`
	TotalTimeMs      int64   `json:"totalTimeMs"`
}

// SupplyReport is the top-level report for supply integrity runs.
type SupplyReport struct {
	RunAt       time.Time      `json:"runAt"`
	Environment string         `json:"environment"`
	Summary     SupplySummary  `json:"summary"`
	Results     []SupplyResult `json:"results"`
}

// NewSupplyReport creates an empty SupplyReport.
func NewSupplyReport(env string, tolerancePct float64) *SupplyReport {
	return &SupplyReport{
		RunAt:       time.Now(),
		Environment: env,
		Summary:     SupplySummary{TolerancePct: tolerancePct},
	}
}

// AddSupplyResult appends a result and updates running totals.
func (r *SupplyReport) AddSupplyResult(res SupplyResult) {
	r.Results = append(r.Results, res)
	if res.Pass {
		r.Summary.PassCount++
	} else {
		r.Summary.FailCount++
		if res.OveragePercent > r.Summary.MaxOverage {
			r.Summary.MaxOverage = res.OveragePercent
		}
	}
}

// FinalizeSupply computes derived summary fields.
func (r *SupplyReport) FinalizeSupply(totalTime time.Duration) {
	r.Summary.TotalChecked = len(r.Results)
	r.Summary.TotalTimeMs = totalTime.Milliseconds()
}

// WriteSupplyJSON saves the supply report as formatted JSON.
func (r *SupplyReport) WriteSupplyJSON(outDir string) (string, error) {
	if err := os.MkdirAll(outDir, 0755); err != nil {
		return "", fmt.Errorf("reporter: create dir: %w", err)
	}
	filename := fmt.Sprintf("supply_%s.json", r.RunAt.Format("2006-01-02_15-04-05"))
	path := filepath.Join(outDir, filename)
	data, err := json.MarshalIndent(r, "", "  ")
	if err != nil {
		return "", fmt.Errorf("reporter: marshal supply: %w", err)
	}
	if err := os.WriteFile(path, data, 0644); err != nil {
		return "", fmt.Errorf("reporter: write supply json: %w", err)
	}
	return path, nil
}

// WriteSupplyHTML saves the supply report as a self-contained HTML file.
func (r *SupplyReport) WriteSupplyHTML(outDir string) (string, error) {
	if err := os.MkdirAll(outDir, 0755); err != nil {
		return "", fmt.Errorf("reporter: create dir: %w", err)
	}
	filename := fmt.Sprintf("supply_%s.html", r.RunAt.Format("2006-01-02_15-04-05"))
	path := filepath.Join(outDir, filename)

	tmpl, err := template.New("supply_report").Funcs(template.FuncMap{
		"printf": fmt.Sprintf,
	}).Parse(supplyHTMLTemplate)
	if err != nil {
		return "", fmt.Errorf("reporter: parse supply template: %w", err)
	}
	f, err := os.Create(path)
	if err != nil {
		return "", fmt.Errorf("reporter: create supply html: %w", err)
	}
	defer f.Close()
	if err := tmpl.Execute(f, r); err != nil {
		return "", fmt.Errorf("reporter: render supply template: %w", err)
	}
	return path, nil
}
...
// -------- HTML TEMPLATE (supply integrity report) --------

var supplyHTMLTemplate = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>Supply Integrity Report · {{.Environment}}</title>
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      --bg: #0d0f14; --surface: #151820; --border: #1e2330;
    ...
    h1 { font-size: 26px; font-weight: 700; background: linear-gradient(135deg,#60a5fa,#a78bfa); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 6px; }
    ...
    th { background: #1a1f2e; color: var(--muted); font-weight: 600; text-transform: uppercase; letter-spacing: .6px; padding: 12px 14px; text-align: right; }
    th:first-child { text-align: left; }
    td { padding: 11px 14px; border-top: 1px solid var(--border); text-align: right; font-family: 'JetBrains Mono', monospace; font-size: 12px; }
    td:first-child { text-align: left; word-break: break-all; }
...
<body>
<div class="wrap">
  <h1>Supply Integrity Report</h1>
  <div class="meta">Environment: {{.Environment}} &nbsp;·&nbsp; {{.RunAt.Format "2006-01-02 15:04:05"}} &nbsp;·&nbsp; Tolerance: {{printf "%.2f" .Summary.TolerancePct}}%</div>

  <div class="cards">
    <div class="card"><div class="card-label">Checked</div><div class="card-value blue">{{.Summary.TotalChecked}}</div></div>
    <div class="card"><div class="card-label">Pass</div><div class="card-value green">{{.Summary.PassCount}}</div></div>
    <div class="card"><div class="card-label">Fail</div><div class="card-value{{if gt .Summary.FailCount 0}} red{{else}} green{{end}}">{{.Summary.FailCount}}</div></div>
    <div class="card"><div class="card-label">Max Overage</div><div class="card-value{{if gt .Summary.MaxOverage 0.0}} red{{else}} green{{end}}">{{printf "%.4f" .Summary.MaxOverage}}%</div></div>
    <div class="card"><div class="card-label">Wall Time</div><div class="card-value blue">{{.Summary.TotalTimeMs}}ms</div></div>
  </div>
  ...
  <table>
    ...
    {{range .Results}}
      <tr>
        <td>{{.EventGroupID}}</td>
        <td>{{printf "%.2f" .MaxSupply}}</td>
        <td>{{printf "%.2f" .FloatingSupply}}</td>
        <td>{{printf "%.2f" .LockedSupply}}</td>
        <td>{{printf "%.2f" .PacksReserve}}</td>
        <td>{{printf "%.2f" .LpReserve}}</td>
        <td>{{printf "%.2f" .AvailableSupply}}</td>
        <td>{{printf "%.2f" .Total}}</td>
        <td>{{printf "%.4f" .OveragePercent}}</td>
        <td><span class="badge {{if .Pass}}badge-pass{{else}}badge-fail{{end}}">{{if .Pass}}PASS{{else}}FAIL{{end}}</span></td>
      </tr>
    {{end}}
    </tbody>
  </table>

  <div class="footer">Generated by code-go-automation &nbsp;·&nbsp; {{.RunAt.Format "2006-01-02 15:04:05"}}</div>
</div>
</body>
</html>`
